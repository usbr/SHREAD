...
Configfile = "config/asset_config.ini"

fileConfig(Configfile)

logger = logging.getLogger('SHREAD Run {}'.format(dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
...
class config_params:
    ...
    def proc_config(self):
        ...
        # open basin_poly
        with open(self.basin_poly_path) as f:
            self.basin_poly = json.load(f)

        # find bounding box for basin_poly
        self.basin_poly_bbox = bbox(list(geojson.utils.coords(self.basin_poly)))
